...
nanovk::Device::Device(const nanovk::ViewProperties& view) {
	...
	for (const auto& avail_q : view.base_device.getQueueFamilyProperties()) {
		bool surface_support = view.base_device.getSurfaceSupportKHR(current_idx, view.surface.get());
		if (!graphics_q && avail_q.queueFlags & vk::QueueFlagBits::eGraphics && surface_support) {
			graphics_q = {
				{},
				avail_q.queueFlags,
				current_idx,
	...
	if (!graphics_q) {
		throw std::runtime_error("Unable to find suitable queue with presentation and graphics");
	}
	...
	for (const auto& avail_extension : view.base_device.enumerateDeviceExtensionProperties()) {
		std::cout << avail_extension.extensionName << "\n";
		for (const auto& enabled : enabled_extensions) {
			if (std::string_view(avail_extension.extensionName).compare(enabled) == 0) {
				missing_extensions--;
			}
		}
	}
	...
	if (missing_extensions > 0) {
		throw std::runtime_error("Not all extensions to enable are supported");
	}
...
void nanovk::Device::createSwapchain(const vk::Extent2D& extent) {
	...
	unsigned int swap_chain_size = surface_properties_.capabilities.minImageCount + 1;
	if (surface_properties_.capabilities.maxImageCount > 0 &&
		swap_chain_size > surface_properties_.capabilities.maxImageCount) {
		swap_chain_size = surface_properties_.capabilities.maxImageCount;
	}
	...
	swapchain_.instance = device_->createSwapchainKHRUnique(swap_chain_create_info);
	swapchain_.images = device_->getSwapchainImagesKHR(swapchain_.instance.get());
	swapchain_.format = format.value().format;
	swapchain_.extent = actual_extent;
}
